...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        if psutil is not None:
            total_memory_usage = self.observations.nbytes + self.actions.nbytes + self.rewards.nbytes + \
                self.dones.nbytes + self.actor_params.nbytes + self.next_actions.nbytes
    ...
    def add(self, obs: np.ndarray, next_obs: np.ndarray, action: np.ndarray, reward: np.ndarray, done: np.ndarray, actor_param: np.ndarray, next_action: np.ndarray) -> None:
        ...
        self.actor_params[self.pos] = np.array(actor_param)
    ...
    def _get_samples(self, batch_inds: np.ndarray, env_inds: np.ndarray = None, env: Optional[VecNormalize] = None) -> ReplayBufferSamples:
        ...
        data = (
            ...
            self.actor_params[batch_inds, env_inds, :],
            self.next_actions[batch_inds, env_inds, :],
        )
